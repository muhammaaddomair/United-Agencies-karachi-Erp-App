...
from services.package_service import list_packages, get_package_items
from ui.theme import COLORS, FONTS, SPACING, get_button_style, create_zebra_table


class PackagePicker(tk.Toplevel):
    ...
    def _build(self):
        ...
        self.empty_label = tk.Label(
            table,
            text="No packages available. Ask admin to add packages.",
            font=FONTS["body"],
    ...
    def _refresh(self):
        ...
        for idx, (package_id, name, _, total) in enumerate(packages):
            tag = "evenrow" if idx % 2 == 0 else "oddrow"
            self.tree.insert("", "end", iid=str(package_id), values=(name, f"{total:,.2f}"), tags=(tag,))
    ...
    def _choose(self, _=None):
        ...
        name = values[0]
        package_price = float(values[1].replace(",", ""))
        items = get_package_items(package_id)
        services = [service_name for _, service_name in items]
        self.on_select(name, package_price, services)
        self.destroy()
